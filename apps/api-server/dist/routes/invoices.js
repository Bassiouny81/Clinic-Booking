...
router.get("/invoices", requireAuth, async (req, res) => {
    try {
        ...
        if (patientId)
            conditions.push(eq(invoicesTable.patientId, patientId));
        ...
        const [invoices, countResult] = await Promise.all([
            ...
            db.select({ count: sql `count(*)` }).from(invoicesTable).where(whereClause),
        ]);
...
router.get("/invoices/:invoiceId/print", async (req, res) => {
    try {
        ...
        if (!invoice)
            return res.status(404).send("<h1>الفاتورة غير موجودة</h1>");
