...
router.get("/doctors/:doctorId/availability", async (req, res) => {
    try {
        ...
        const existingAppointments = await db
            ...
            .where(and(eq(appointmentsTable.doctorId, doctorId), gte(appointmentsTable.scheduledAt, dayStart), lte(appointmentsTable.scheduledAt, dayEnd), isNull(appointmentsTable.deletedAt)));
        const slots = [];
        const workStart = 9;
        const workEnd = 17;
