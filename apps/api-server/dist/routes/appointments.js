...
import { appointmentsTable, patientsTable, doctorsTable, servicesTable, appointmentTypesTable, } from "@workspace/db/schema";
...
async function enrichAppointments(appointments) {
    ...
    const serviceMap = Object.fromEntries(services.map((s) => [s.id, { ...s, price: parseFloat(s.price), vatRate: parseFloat(s.vatRate) }]));
...
router.get("/appointments", requireAuth, async (req, res) => {
    try {
        ...
        const [appointments, countResult] = await Promise.all([
            ...
            db.select({ count: sql `count(*)` }).from(appointmentsTable).where(whereClause),
        ]);
...
router.post("/appointments", requireAuth, async (req, res) => {
    try {
        const { patientId, doctorId, serviceId, appointmentTypeId, scheduledAt, durationMinutes = 60, mode, notes } = req.body;
...
router.get("/dashboard/stats", requireAuth, async (req, res) => {
    try {
        ...
        const [todayResult, weekResult, patientResult, revenueResult, pendingInvoicesResult, upcomingAppointments,] = await Promise.all([
            db
                .select({ count: sql `count(*)` })
                .from(appointmentsTable)
                .where(and(gte(appointmentsTable.scheduledAt, todayStart), lte(appointmentsTable.scheduledAt, todayEnd), isNull(appointmentsTable.deletedAt))),
            db
                .select({ count: sql `count(*)` })
                .from(appointmentsTable)
                .where(and(gte(appointmentsTable.scheduledAt, todayStart), lte(appointmentsTable.scheduledAt, weekEnd), isNull(appointmentsTable.deletedAt))),
            db
                .select({ count: sql `count(*)` })
                .from(patientsTable)
      ...
      `),
            db
                .select()
                .from(appointmentsTable)
                .where(and(gte(appointmentsTable.scheduledAt, now), isNull(appointmentsTable.deletedAt), eq(appointmentsTable.status, "scheduled")))
                .limit(5)
        ...
        res.json({
            ...
            monthRevenue: Number(revenueResult.rows[0]?.revenue || 0),
            pendingInvoices: Number(pendingInvoicesResult.rows[0]?.count || 0),
