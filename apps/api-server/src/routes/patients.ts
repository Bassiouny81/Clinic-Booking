...
router.get("/patients", requireAuth, async (req, res) => {
  try {
    ...
    if (search) {
      conditions.push(
        sql`(${patientsTable.nameAr} ILIKE ${`%${search}%`} OR ${patientsTable.phone} ILIKE ${`%${search}%`} OR ${patientsTable.nameEn} ILIKE ${`%${search}%`})`
      );
    }
...
router.post("/patients", async (req, res) => {
  try {
    const { nameAr, nameEn, phone, email, dateOfBirth, gender, nationalId, medicalHistory, allergies, metadata } =
      req.body;
